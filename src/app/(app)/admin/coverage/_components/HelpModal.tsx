    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-6">
      <div className="flex flex-col w-full max-w-[760px] max-h-[85vh] rounded-[10px] border border-hair bg-card shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-hair px-5 py-3 shrink-0">
          <h3 className="text-[14px] font-medium text-ink">Help — Pipeline Coverage</h3>
          <button
            onClick={onClose}
        ...
        {/* Tab strip */}
        <div className="flex border-b border-hair px-5 shrink-0">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6 text-[13px] leading-relaxed text-ink-3">
          ...
          {tab === "admins" && (
            <>
              <section className="space-y-3">
                <h4 className="text-[11px] font-semibold uppercase tracking-wider text-ink">L1 — Transcript / PPT / Annual Report Extraction</h4>

                <div>
                  <p className="text-ink font-medium mb-0.5">What it does</p>
                  <p>On-demand, one-off signal extraction — there&rsquo;s no cron/auto mode, every run is triggered manually for a chosen ticker set. <span className="font-medium text-ink">Preview</span> is read-only and free to repeat, but it&rsquo;s optional — <span className="font-medium text-ink">Run</span> is enabled from the start and dispatches real, billable LLM extraction jobs with whatever options are currently set, whether or not you&rsquo;ve previewed them.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Which companies?</p>
                  <p><span className="font-medium text-ink">Default list</span> falls back to the backend&rsquo;s built-in ticker list. <span className="font-medium text-ink">Manual tickers</span> lets you type an explicit set for this run only. <span className="font-medium text-ink">Saved group</span> reuses a Company Group. <span className="font-medium text-ink">All companies</span> scans everything (~2,000 tickers) — pair with Start From to resume a partial run.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Extraction options</p>
                  <p><span className="font-medium text-ink">Start From</span> is an alphabetical cursor to resume a long run. <span className="font-medium text-ink">Limit</span> and <span className="font-medium text-ink">Latest</span> both cap to the N most-recent calls/reports per ticker — Latest wins if both are set. Leave both blank and Preview shows only the 20 most recent calls per ticker (the true total is still shown alongside) — set either one to look further back. <span className="font-medium text-ink">Force re-extract</span> re-runs tickers that already have signals, discarding the old ones (off by default, so already-extracted tickers are normally skipped). <span className="font-medium text-ink">Annual reports only</span> and <span className="font-medium text-ink">Skip annual reports</span> are mutually exclusive toggles for which source types run.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Preview pagination</p>
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">Export CSV</p>
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">Run History errors</p>
                  <p>When a ticker&rsquo;s per-ticker row in Run History shows a non-zero <span className="font-medium text-ink">Failed</span> count, that count becomes a clickable expander revealing the specific document(s) that failed — source (transcript/PPT/annual report), fiscal year/quarter, and the error message. A ticker can fail on more than one document in the same run.</p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                <h4 className="text-[11px] font-semibold uppercase tracking-wider text-ink">L2 — Skill Dispatch</h4>
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">Group Config Tag</p>
                  <p>Picking <span className="font-medium text-ink">Saved group</span> surfaces a config-tagging control right there in the dispatch flow, so you can tag or retag that group with the skill config it should resolve to without leaving the page. It&rsquo;s always editable, even if the group is already tagged.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Multiple skills</p>
                  <p>The Skill field is a checkbox list, not a single picker — check as many as you want, or use <span className="font-medium text-ink">Select all</span>. Preview always queries just the first selected skill (one <code>/preview</code> call, since it only accepts a single slug). Run fans out to every selected skill at once — one <code>/run</code> POST per skill, fired concurrently, not one at a time — and each gets its own row in Run History.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Historic mode / Force</p>
                  <p>Both are <span className="font-medium text-ink">Run only</span> — Preview ignores them entirely. <span className="font-medium text-ink">Historic</span> runs the full base-context build instead of incremental; <span className="font-medium text-ink">Force</span> bypasses the cache. Historic mode is currently locked on (checked and disabled) — incremental bulk dispatch is disabled for now.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Preview</p>
                  <p>A fast, single-query signal-availability report — not a per-ticker prompt build, so it&rsquo;s quick even for large groups. Shows transcript/PPT/annual-report totals per ticker, expandable to a per fiscal-year/quarter breakdown. Use <span className="font-medium text-ink">Sort by least covered</span> to surface the biggest coverage gaps for a given source first — note this only sorts within the current page, not across the whole ticker set. Page controls appear once a preview spans more than one page; paging doesn&rsquo;t affect Run.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Export CSV vs JSON Preview</p>
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">Run History caveat</p>
              ...
              <section className="space-y-3">
                <h4 className="text-[11px] font-semibold uppercase tracking-wider text-ink">Daily Runs — Prowess Daily &amp; BSE Discovery</h4>

                <div>
                  <p className="text-ink font-medium mb-0.5">Prowess Daily</p>
                  <p>Not implemented yet — placeholder section, coming later.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">BSE Discovery — what it does</p>
                  <p><span className="font-medium text-ink">Run Discovery</span> scrapes BSE and resolves cover-letter PDFs in the background (~1-2 min); an optional Lookback Days narrows how far back it looks. Nothing lands in the real tables automatically — every discovered URL passes through an admin&rsquo;s eyes first.</p>
                </div>
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">Company / Fiscal Year / Quarter</p>
              ...
              <section className="space-y-3">
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">Manual vs Dynamic</p>
                  <p><span className="font-medium text-ink">Manual</span> is a fixed, explicit list of tickers. <span className="font-medium text-ink">Dynamic</span> resolves live from a flat set of filters — Alphabet range, Transcript/PPT/Annual Report status (present, not yet extracted, or already extracted), Market cap, and Industries. Every enabled filter is ANDed together — there&rsquo;s no any/all toggle.</p>
                </div>

                <div>
                  <p className="text-ink font-medium mb-0.5">Transcript / PPT recency window</p>
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">config_key</p>
                ...
                <div>
                  <p className="text-ink font-medium mb-0.5">Overlapping groups</p>
                  <p>A ticker can belong to more than one tagged group at once — there&rsquo;s no exclusivity check when you save a group. When that happens, the config from whichever of those groups was <span className="font-medium text-ink">most recently updated</span> wins for that ticker, not any fixed priority order. If a ticker&rsquo;s L2 results don&rsquo;t match the group you expect, check whether it also sits in another tagged group that was edited more recently.</p>
                </div>
              </section>
            </>
          )}
          ...
          {tab === "faq" && (
            <section className="space-y-4">
              <div>
                <p className="text-ink font-medium mb-0.5">Q: I clicked Run on L1 or L2 — can I stop, pause, or resume it from this page?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: I triggered a run but Run History still shows nothing, or hasn&rsquo;t updated after a while — is polling broken?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: L2 Run History shows a ticker as &ldquo;queued&rdquo; or the run as &ldquo;completed&rdquo; — does that mean it actually worked?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: I closed the tab or navigated away right after clicking Run — did that cancel anything?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: Preview passed with no errors, but Run gives a 400 for some L2 tickers — what happened?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: BSE Discovery&rsquo;s run is taking longer than the usual ~1-2 minutes, or seems stuck on &ldquo;running&rdquo; — what do I do?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: I approved a BSE Discovery URL with the wrong ticker or fiscal year — can I fix it?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: A discovered URL I never approve — does it expire, or get cleaned up automatically?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: I tagged a Company Group&rsquo;s config_key from the L2 tab — does that change what L1 or L3 do with that same group?</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Q: I edited a dynamic (filter-based) group&rsquo;s filters after already running Preview — do I need to Preview again before Run?</p>
          ...
          {tab === "developers" && (
            <section className="space-y-3">
              <div>
                <p className="text-ink font-medium mb-1">L1 vs L2 preview mechanics</p>
                <table className="w-full text-[11px] border-collapse">
                  <thead>
                    <tr className="border-b border-hair">
                      <th className="text-left py-1 pr-2 font-semibold text-ink-3"></th>
                      <th className="text-left py-1 pr-2 font-semibold text-ink-3">L1 Preview</th>
                      <th className="text-left py-1 font-semibold text-ink-3">L2 Preview</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-hair">
                    <tr>
                      <td className="py-1 pr-2 text-ink">Query pattern</td>
                      <td className="py-1 pr-2">Per-ticker signal presence</td>
                      <td className="py-1">One batched query for the whole set</td>
                    </tr>
                    <tr>
                      <td className="py-1 pr-2 text-ink">Respects Historic/Force?</td>
                      <td className="py-1 pr-2">n/a</td>
                      <td className="py-1">No — Run only</td>
                    </tr>
                    <tr>
                      <td className="py-1 pr-2 text-ink">CSV export scope</td>
                    ...
                    <tr>
                      <td className="py-1 pr-2 text-ink">Pagination</td>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Preview/CSV response caching</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">Run History polling</p>
              ...
              <div>
                <p className="text-ink font-medium mb-0.5">L2 dispatch vs skill outcome</p>
