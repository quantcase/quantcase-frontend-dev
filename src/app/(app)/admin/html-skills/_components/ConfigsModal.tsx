...
export function ConfigsModal({ configs, onSelect, onCreate, onDelete, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-6">
      <div className="flex flex-col w-full max-w-[440px] max-h-[70vh] rounded-[10px] border border-[var(--qc-border-default)] bg-white shadow-xl overflow-hidden">
        {/* Header */}
        ...
        <p className="px-5 pt-3 text-[11px] text-[#888888]">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          <button
            ...
            className="flex w-full items-center justify-between rounded-md px-3 py-2 text-left hover:bg-[#FAFAFA] transition-colors"
          >
            <span className="text-[12px] font-medium text-[var(--qc-ink)]">Default (skill&rsquo;s own settings)</span>
          </button>

          {configs.map((cfg) => (
            <div key={cfg.key} className="flex items-center gap-1 rounded-md hover:bg-[#FAFAFA] transition-colors">
              <button
              ...
              >
                ...
                <span className="text-[10px] font-mono text-[#888888]">{cfg.key}</span>
              </button>
              <button
                onClick={() => onDelete(cfg.key)}
                title="Delete config"
                className="flex items-center justify-center size-7 mr-2 rounded text-[#888888] hover:text-red-600 transition-colors shrink-0"
              >
          ...
          {configs.length === 0 && !creating && (
            <p className="px-3 py-4 text-[12px] text-[#888888]">No saved configs yet.</p>
          )}
        </div>
          ...
          {creating ? (
            <div className="space-y-2">
              <input
                ...
                className="w-full rounded-md border border-[var(--qc-border-default)] bg-white px-3 py-2 text-[12px] font-mono text-[var(--qc-ink)] outline-none focus:ring-1 focus:ring-[var(--qc-ink)]"
              />
              <input
                type="text"
                value={newName}
                ...
                className="w-full rounded-md border border-[var(--qc-border-default)] bg-white px-3 py-2 text-[12px] text-[var(--qc-ink)] outline-none focus:ring-1 focus:ring-[var(--qc-ink)]"
              />
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="rounded-md px-3 py-1.5 text-[12px] font-medium text-[#888888] hover:text-[var(--qc-ink)] transition-colors"
                >
                  Cancel
                </button>
              </div>
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-1.5 rounded-md border border-dashed border-[var(--qc-border-default)] px-3 py-2 text-[12px] font-medium text-[#888888] hover:text-[#0F172B] hover:border-[#0F172B] transition-colors"
            >
