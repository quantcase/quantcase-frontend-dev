...
export function PinnedBaseModal({ slug, ticker, onClose }: Props) {
  ...
  async function handleTogglePin(row: OutputHistoryRow) {
    setActionRowId(row.id);
    setError(null);
    try {
      if (row.is_pinned_base) {
        const res = await fetch(`${BACKEND_URL}${API_BASE}/${slug}/outputs/${ticker}/pin`, { method: "DELETE" });
        if (!res.ok) throw new Error(`${res.status}`);
      } else {
        const res = await fetch(`${BACKEND_URL}${API_BASE}/${slug}/outputs/${ticker}/pin`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ fiscal_year: row.fiscal_year, quarter: row.quarter, historic: row.is_historic }),
        });
        if (!res.ok) throw new Error(`${res.status}`);
      }
      loadHistory();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Pin action failed");
    } finally {
      setActionRowId(null);
    }
  }
      ...
      <div className="flex flex-col w-full max-w-[680px] max-h-[80vh] rounded-[10px] border border-[var(--qc-border-default)] bg-white shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-[var(--qc-border-default)] px-5 py-3 shrink-0">
          <div className="flex items-center gap-3">
            <h3 className="text-[14px] font-medium text-[var(--qc-ink)]">Pin Base Output</h3>
          ...
          {!loading && rows.length > 0 && (
            <table className="w-full text-[12px]">
              ...
              <tbody className="divide-y divide-[#E2E2E2]">
                {rows.map((row) => {
                  const isPinned = row.is_pinned_base;
                    ...
                    <tr key={row.id} className="hover:bg-[#FAFAFA] transition-colors">
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          {isPinned && (
                            <span className="rounded-sm px-2 py-0.5 text-[10px] font-medium whitespace-nowrap bg-[#0F172B] text-white">
                              PINNED
                            </span>
                          )}
                        </div>
