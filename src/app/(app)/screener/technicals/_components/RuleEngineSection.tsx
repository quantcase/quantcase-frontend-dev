...
function MetricPill({ label, value, valueColor }: { label: string; value: string; valueColor?: string }) {
  return (
    ...
    >
      <span style={{ fontSize: "11.5px", color: "var(--qc-ink-2)", fontWeight: "var(--qc-w-medium)", textTransform: "uppercase", letterSpacing: "0.06em", fontFamily: "var(--qc-font-mono)" }}>{label}</span>
      <span style={{ fontSize: "13.5px", fontWeight: "var(--qc-w-semi)", color: valueColor ?? "var(--qc-ink)", fontFamily: "var(--qc-font-sans)" }}>{value}</span>
    </span>
  );
}
...
function EngineCard({
  ...
  return (
    <div
      className="rounded-[10px] border px-4.5 py-4 space-y-3.5"
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-baseline gap-2.5">
          <span
            className="font-mono text-[13px] uppercase tracking-[0.10em] font-semibold"
            style={{ color: "var(--qc-ink)" }}
          >{title}</span>
          {subtitle && (
            <span style={{ fontSize: "13.5px", color: "var(--qc-ink-2)", fontWeight: "var(--qc-w-regular)", fontFamily: "var(--qc-font-sans)" }}>{subtitle}</span>
          )}
        </div>
        {badgeLabel && (
          <span
            className="shrink-0 inline-flex items-center rounded-[4px] px-2.5 py-1 font-mono text-[12px] uppercase tracking-[0.08em] font-semibold"
      ...
      {(output || emptyOutput) && (
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5">
            <TrendingUp className="h-4 w-4" style={{ color: "var(--qc-ink-2)" }} />
            <span style={{ fontSize: "12px", fontWeight: "var(--qc-w-semi)", color: "var(--qc-ink-2)", textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: "var(--qc-font-mono)" }}>
              Interpretation
            </span>
          </div>
          {output ? (
            <p style={{ fontSize: "15px", color: "var(--qc-ink)", lineHeight: 1.65, fontFamily: "var(--qc-font-sans)" }}>
              {output.split("\n").map((line, i, arr) => (
                <span key={i}>{line}{i < arr.length - 1 && <br />}</span>
              ))}
            </p>
          ) : (
            <p style={{ fontSize: "14px", color: "var(--qc-ink-2)", fontFamily: "var(--qc-font-sans)" }}>{emptyOutput}</p>
          )}
        </div>
      )}
      ...
      {watchout && (
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5">
            <AlertCircle className="h-4 w-4" style={{ color: "var(--qc-warn)" }} />
            <span style={{ fontSize: "12px", fontWeight: "var(--qc-w-semi)", color: "var(--qc-ink-2)", textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: "var(--qc-font-mono)" }}>
              Watchouts
            </span>
          </div>
          <div className="rounded-[8px] px-3.5 py-2.5" style={{ background: "var(--qc-warn-soft)" }}>
            <p style={{ fontSize: "15px", color: "var(--qc-warn)", lineHeight: 1.65, fontFamily: "var(--qc-font-sans)" }}>{watchout}</p>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {wyckoffGrowthWarning && (
        <div className="flex items-start gap-2.5 rounded-[8px] px-3.5 py-2.5" style={{ background: "var(--qc-warn-soft)" }}>
          <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" style={{ color: "var(--qc-warn)" }} />
          <p style={{ fontSize: "15px", color: "var(--qc-warn)", lineHeight: 1.6, fontFamily: "var(--qc-font-sans)" }}>
            {wyckoffGrowthWarning}
          </p>
        </div>
      )}
