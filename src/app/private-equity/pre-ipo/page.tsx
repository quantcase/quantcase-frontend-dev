  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-4">
      <div
        ...
        style={{ borderColor: drag ? "var(--qc-text-heading)" : "var(--qc-border-default)", background: drag ? "var(--qc-surface-panel)" : "var(--qc-surface-base)" }}
      >
        <input
          ref={inputRef}
          type="file"
        ...
        {file ? (
          <>
            <FileText className="size-8" style={{ color: "var(--qc-text-heading)" }} />
            <div className="text-center">
              <p className="text-[14px] font-medium" style={{ color: "var(--qc-text-heading)" }}>{file.name}</p>
              <p className="text-[12px] mt-0.5" style={{ color: "var(--qc-text-muted)" }}>
          ...
          <>
            <Upload className="size-8" style={{ color: "var(--qc-text-muted)" }} />
            <div className="text-center">
              <p className="text-[14px] font-medium" style={{ color: "var(--qc-text-heading)" }}>Drop your DRHP here</p>
              <p className="text-[12px] mt-0.5" style={{ color: "var(--qc-text-muted)" }}>PDF or .txt · max 50 MB</p>
            </div>
          </>
        )}
      </div>
...
function PastAnalysesList({ items, onSelect }: { items: DrhpRecord[]; onSelect: (id: string) => void }) {
  ...
  return (
    <div className="mt-12">
      <p className="text-[11px] font-bold uppercase tracking-[0.12em] mb-4" style={{ color: "var(--qc-text-muted)" }}>
        Past Analyses
      </p>
        ...
        {items.map((record) => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-4 rounded-[10px] border border-[var(--qc-border-default)] bg-white px-4 py-3.5 text-left hover:border-[var(--qc-border-active)] transition-colors group"
            >
              <div className="flex-1 min-w-0">
                <p className="text-[14px] font-medium truncate" style={{ color: "var(--qc-text-heading)" }}>
              ...
              <ChevronRight className="size-4 opacity-40 group-hover:opacity-100 transition-opacity flex-shrink-0" style={{ color: "var(--qc-text-muted)" }} />
            </button>
          );
        })}
      </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-8 pb-16">
        ...
        <div className="mb-8">
          <p className="text-[11px] font-bold uppercase tracking-[0.12em] mb-1" style={{ color: "var(--qc-text-muted)" }}>
            Private Equity · Pre-IPO
          </p>
          <h1 className="text-[28px] font-medium leading-tight" style={{ color: "var(--qc-text-heading)" }}>DRHP Analyser</h1>
          <p className="text-[14px] mt-2" style={{ color: "var(--qc-text-muted)" }}>
