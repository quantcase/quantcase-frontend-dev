...
// Decorative "scanning radar" illustration built from the same design tokens as
// the real scorecard radar — signals that analysis is in flight, not missing.
function RadarIllustration() {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ overflow: "visible" }}>
      ...
      {/* Sweeping wedge — rotates to read as an active scan */}
      <motion.g
      ...
      >
        <defs>
          <linearGradient id="empty-radar-sweep" x1="50%" y1="50%" x2="100%" y2="0%">
...
export function InsightEmptyState({ type, company }: { type: InsightType; company?: string | null }) {
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-6 py-16" style={{ background: "var(--qc-bg)" }}>
      ...
      >
        ...
        <p style={{ fontSize: "var(--qc-fz-13)", color: "var(--qc-ink-3)", lineHeight: 1.6, margin: 0 }}>
          We haven&apos;t completed the {TYPE_LABELS[type].toLowerCase()} analysis
