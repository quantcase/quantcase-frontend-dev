...
import type { MqiScore, InvestmentThesis } from "@/types/management";

interface QcScoreHeroCardProps {
  mqiScore: MqiScore;
  investmentThesis?: InvestmentThesis | null;
  /** Company thesis headline — e.g. "State-owned E&P giant with a strong moat" */
  thesisHeadline?: string | null;
  /** Key P/E ratio for right-card footer anchor */
  peRatio?: number | null;
  /** OPM % for right-card footer anchor */
  opm?: number | null;
}
...
function getRatingLabel(score: number, maxScore: number): string {
  ...
  if (pct >= 0.80) return "Strong Buy";
  if (pct >= 0.65) return "Buy";
  if (pct >= 0.50) return "Hold";
  if (pct >= 0.35) return "Underperform";
  return "Sell";
}
...
export function QcScoreHeroCard({
  ...
  const score = mqiScore.total;
  const maxScore = 100;
  const ratingLabel = getRatingLabel(score, maxScore);

  // Build legend from thesis dimensions
  const dims = mqiScore.dimensions;
  const legendItems = [
    { label: "Guidance Accuracy", color: "var(--qc-up, #1F7A4A)", muted: false },
    { label: "Red Flags", color: "var(--qc-blue, #2563EB)", muted: false },
    { label: "Investment Thesis", color: "var(--qc-warn, #B4731A)", muted: dims.investment_thesis.score < dims.investment_thesis.max * 0.4 },
    { label: "Promoter Activity", color: "var(--qc-down, #B23A2F)", muted: dims.promoter_activity.score < dims.promoter_activity.max * 0.4 },
  ];

  // Static SVG path data — score line shape based on score position
  // Score maps to y: score 100 = y20, score 0 = y220. y = 220 - (score/100)*200
  const scoreY = Math.round(220 - (score / 100) * 200);
  const startY = Math.min(220, scoreY + 30);

  // Thesis headline — fallback to bull_case[0]
  const headline = thesisHeadline
    ?? investmentThesis?.bull_case?.[0]
    ?? "Management credibility score across guidance, thesis and promoter activity.";

  const bullPoints = investmentThesis?.bull_case?.slice(0, 2) ?? [];
  const bearPoints = investmentThesis?.bear_case?.slice(0, 1) ?? [];
  ...
  return (
    ...
    >
      {/* LEFT: hero-chart — QC Score with band chart */}
      <section
        style={{
      ...
      >
        ...
        <div style={{ position: "relative", margin: "0 -4px" }}>
          <svg viewBox="0 0 920 240" width="100%" height="240" preserveAspectRatio="none">
            ...
            {/* sector median (gray dashed) */}
            <path
              d="M0,112 C80,108 160,104 240,100 C320,95 400,90 480,88 C560,86 640,85 720,84 C800,83 880,82 920,82"
              fill="none" stroke="#B8B5AB" strokeWidth="1.6" strokeDasharray="5 4"
            />
            {/* stock score trajectory */}
            <path
              d={`M0,${startY} C70,${startY - 8} 130,${startY + 2} 200,${Math.min(220, startY + 20)} C270,${Math.min(220, startY + 38)} 330,${Math.min(220, startY + 40)} 400,${Math.min(220, startY + 32)} C470,${scoreY + 24} 530,${scoreY + 15} 600,${scoreY + 12} C670,${scoreY + 9} 730,${scoreY + 22} 800,${scoreY + 30} C860,${scoreY + 36} 900,${scoreY + 32} 920,${scoreY + 28}`}
          ...
          {annotVisible && (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 18, fontWeight: 500, letterSpacing: "-0.015em", fontVariantNumeric: "tabular-nums" }}>{score}</span>
                <span
                  style={{
                    fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, fontWeight: 600,
                    padding: "3px 7px", borderRadius: 5,
                    background: "#DFF0A6", color: "#40580F", letterSpacing: "0.04em",
                  }}
                >
                  {ratingLabel}
                </span>
              ...
              <div style={{ fontSize: 11.5, color: "var(--qc-text-muted)", marginTop: 4, lineHeight: 1.35 }}>
                {mqiScore.label} management quality rating
              </div>
          ...
          <div
            style={{
              ...
              color: "var(--qc-text-muted)", letterSpacing: "0.08em",
              textTransform: "uppercase", padding: "6px 4px 0",
            }}
          >
            {["Q3 FY23", "Q4 FY23", "Q1 FY24", "Q2 FY24", "Q3 FY24", "Today"].map((label) => (
      ...
      {/* RIGHT: insight-card — Investment Thesis (lime gradient) */}
      <aside
        style={{
      ...
      >
        ...
        {/* insight-plot: operating margin stability chart */}
        <div style={{ flex: 1, minHeight: 120, position: "relative", margin: "0 -4px" }}>
          <svg viewBox="0 0 360 140" width="100%" height="140" preserveAspectRatio="none">
            <defs>
              <linearGradient id="insight-fill-mgmt" x1="0" x2="0" y1="0" y2="1">
            ...
            <g stroke="#9CB844" strokeWidth="1.4" opacity=".38">
              {[18,36,54,72,90,108,126,144,162,180,198,216,234,252,270,288,306,324,342].map((x, i) => {
                const yTop = 72 - i * 1.8;
                return <line key={x} x1={x} y1="130" x2={x} y2={Math.max(20, yTop)} />;
              })}
            </g>
            {/* score trend line */}
            <path
              d="M0,85 C50,80 100,75 150,70 C200,65 260,55 320,40 L360,30"
              fill="none" stroke="#0E0E0C" strokeWidth="2"
            />
            <path
              d="M0,85 C50,80 100,75 150,70 C200,65 260,55 320,40 L360,30 L360,140 L0,140 Z"
              fill="url(#insight-fill-mgmt)"
            />
          </svg>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 12, marginTop: 6 }}>
          <div>
            <div style={{ fontSize: 26, fontWeight: 500, letterSpacing: "-0.02em", color: "#0E0E0C", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>
              {score}<span style={{ fontSize: 16, marginLeft: 1 }}>/100</span>
            </div>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 26, fontWeight: 500, letterSpacing: "-0.02em", color: "#0E0E0C", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>
              {bearPoints.length > 0
                ? <span style={{ fontSize: 16, fontWeight: 500 }}>{mqiScore.label}</span>
                : <span style={{ fontSize: 16, fontWeight: 500 }}>{mqiScore.label}</span>
              }
            </div>
