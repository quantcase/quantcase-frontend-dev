...
export function PortfolioDropdown({ portfolios, selected, onChange }: Props) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-3 rounded-[10px] border border-hair bg-card px-4 py-3 hover:bg-secondary transition-colors"
        style={{ minWidth: 280 }}
      >
        ...
        <ChevronDown
          className="size-4 text-ink-3 transition-transform"
      ...
      {open && (
        <div
          className="absolute top-full left-0 mt-1 z-50 rounded-[10px] border border-hair bg-card shadow-lg overflow-hidden"
          style={{ minWidth: 280 }}
        >
          {portfolios.map((p) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 hover:bg-secondary transition-colors text-left border-b border-hair last:border-0"
            >
              ...
              {p.id === selected.id && (
                <ShieldCheck className="size-4 text-up flex-shrink-0" />
              )}
            </button>
          ))}
        </div>
