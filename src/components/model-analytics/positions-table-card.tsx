...
function PositionRow({ pos, rank }: { pos: PortfolioData["positions"][0]; rank: number }) {
  ...
  return (
    ...
    >
      <span style={{ fontSize: 11, color: "#888888", fontVariantNumeric: "tabular-nums" }}>{rank}</span>
      <div className="min-w-0">
        <div style={{ fontSize: 13, fontWeight: 500, color: "#0F172B", lineHeight: 1.3 }} className="truncate">
          {pos.company}
        </div>
        <div style={{ fontSize: 11, color: "#888888" }}>{pos.ticker}</div>
      </div>
      <span
        className="rounded-sm px-1.5 py-0.5 text-center"
        style={{ fontSize: 10, color: "#888888", background: "#F5F5F5", textTransform: "uppercase", letterSpacing: "0.04em" }}
      >
      ...
      <span
        ...
        style={{ fontSize: 13, fontWeight: 500, color: "#0F172B", fontVariantNumeric: "tabular-nums" }}
      >
        {pos.allocation}%
      </span>
    </div>
    ...
    <div className="rounded-[10px] border border-[#E2E2E2] bg-[#F5F5F5] p-2">
      <div className="rounded-[10px] bg-white p-4">
        <div className="flex items-center justify-between mb-4">
          <span style={{ fontSize: 14, fontWeight: 600, color: "#0F172B" }}>Equity Positions</span>
        ...
        >
          {["#", "Company", "Class", "IC Score", "Alloc"].map((h) => (
            <span
              ...
              style={{ fontSize: 10, color: "#888888", textTransform: "uppercase", letterSpacing: "0.06em" }}
            >
              {h}
            </span>
          ))}
        ...
        {sorted.length === 0 && (
          <div className="py-8 text-center" style={{ color: "#888888", fontSize: 13 }}>
