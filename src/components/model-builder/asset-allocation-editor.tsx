...
function Toggle({ on, onClick }: { on: boolean; onClick: () => void }) {
  return (
    <button
      ...
      style={{ background: on ? "#475569" : "#E2E2E2" }}
    >
      <span
          ...
          <div key={item.key} className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: item.enabled ? "#0F172B" : "#888888" }}>
                    {def?.label ?? item.key}
                  </p>
                  {def && (
                    <p className="text-xs" style={{ color: "#888888" }}>{def.description}</p>
                  )}
                </div>
              </div>
              <div className="text-right shrink-0 ml-4">
                {item.enabled ? (
                  <>
                    <p className="text-sm font-semibold" style={{ color: "#0F172B" }}>{item.pct}</p>
  ...
  return (
    <div className="border-b border-[#E2E2E2] last:border-0">
      <div className="flex items-center justify-between py-3">
        <div>
          <p className="text-sm font-semibold" style={{ color: "#0F172B" }}>{assetLabel}</p>
              ...
              <div key={sub.key} className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold" style={{ color: sub.enabled ? "#0F172B" : "#888888" }}>
                        {sub.label}
                      </p>
                      {sub.description && (
                  ...
                  <div className="text-right shrink-0 ml-4">
                    {sub.enabled ? (
                      <>
                        <p className="text-sm font-semibold" style={{ color: "#0F172B" }}>{sub.pct}</p>
                        <p className="text-xs" style={{ color: "#888888" }}>{formatCapital(subAmount)}</p>
                      </>
                    ) : (
                      <p className="text-xs" style={{ color: "#888888" }}>Off</p>
                    )}
