...
function FormLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[10px] uppercase tracking-wider font-semibold mb-1.5" style={{ color: "#888888" }}>
      {children}
    </p>
  );
}
...
export function TextInput({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-[#E2E2E2] px-3 py-2 bg-white focus-within:border-[#0F172B] transition-all">
      {prefix && <span className="text-sm shrink-0" style={{ color: "#888888" }}>{prefix}</span>}
      <input
        type={type}
        value={value}
        ...
        style={{ color: value ? "#0F172B" : undefined }}
      />
      {suffix && <span className="text-sm shrink-0" style={{ color: "#888888" }}>{suffix}</span>}
    </div>
  );
}
  ...
  return (
    <div className="relative">
      <select
        ...
        style={{ color: value ? "#0F172B" : "#888888" }}
      >
      ...
      <ChevronDown className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "#888888" }} />
    </div>
  );
}
    ...
    <div className="flex gap-1 p-1 rounded-full border border-[#E2E2E2] bg-[#F5F5F5] w-fit">
      {options.map((o) => (
        <button
          ...
          style={{
            background: value === o.value ? "#0F172B" : "transparent",
            color:      value === o.value ? "#fff" : "#888888",
          }}
        >
          {o.label}
        </button>
...
export function SectionBox({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-[10px] border border-[#E2E2E2] bg-[#F5F5F5] p-2">
      <p className="text-[11px] font-semibold uppercase tracking-wider px-2 pt-1 pb-3" style={{ color: "#888888" }}>{title}</p>
...
function MethodCard({
  ...
  return (
    ...
    >
      <p className="text-[12px] font-bold mb-1" style={{ color: "#0F172B" }}>{label}</p>
      <p className="text-[11px] leading-snug" style={{ color: "#888888" }}>{description}</p>
    </button>
  );
}
    ...
    <div className="space-y-4">
          ...
          {MILESTONE_DEFS.map((m) => {
            ...
            return (
              <div
                ...
                style={{
                  borderColor: active ? m.color : "#E2E2E2",
              ...
              >
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {m.tag && (
                      <span className="text-[11px] font-semibold" style={{ color: "#0F172B" }}>{m.tag}</span>
                    )}
                  </div>
                  <p className="text-[11px]" style={{ color: "#888888" }}>{m.age}</p>
                </div>
                <button
                  type="button"
                  onClick={() => onChange({ ...value, [m.key]: !active })}
                  className="relative rounded-full transition-all"
                  style={{ width: 40, height: 22, background: active ? m.color : "#E2E2E2" }}
                >
                  <span
      ...
      <SectionBox title="Age → phase engine">
        <div className="space-y-2">
          ...
          <div className="flex text-[9px] justify-between" style={{ color: "#888888" }}>
          ...
          {[
            ...
            <div key={p.phase} className="rounded-lg border border-[#E2E2E2] p-3">
              <div className="flex items-center justify-between mb-1">
                <p className="text-[11px] font-semibold" style={{ color: "#0F172B" }}>{p.phase}</p>
                <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-[#F5F5F5]" style={{ color: "#888888" }}>{p.age}</span>
              </div>
              <p className="text-[11px] leading-snug" style={{ color: "#888888" }}>{p.desc}</p>
            </div>
          ))}
        </div>
      </SectionBox>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-[#E2E2E2] bg-[#F5F5F5] px-4 py-3">
        <p className="text-[10px] uppercase tracking-wider font-medium mb-0.5" style={{ color: "#888888" }}>Configuring SWP for goal</p>
        <p className="text-sm font-semibold" style={{ color: "#0F172B" }}>{goalLabel}</p>
      </div>
