...
const SWOT_CONFIG = [
  ...
  { key: "threats" as const, label: "Threats", icon: Zap, colorVar: "var(--qc-ink-2)" },
] as const;
  ...
  return (
    <div className="space-y-4">
      <div style={{ borderRadius: 10, border: "1px solid var(--qc-hair)", background: "var(--qc-card)", overflow: "hidden" }}>
        <div className="grid grid-cols-2" style={{ borderColor: "var(--qc-hair)" }}>
          {SWOT_CONFIG.map(({ key, label, icon: Icon, colorVar }, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  borderTop: isBottomRow ? "1px solid var(--qc-hair)" : "none",
                  borderRight: idx % 2 === 0 ? "1px solid var(--qc-hair)" : "none",
                }}
              >
