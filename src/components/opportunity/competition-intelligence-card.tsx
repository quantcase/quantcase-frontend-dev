...
import { BarChart3, Info, ArrowRight, ShieldAlert, TrendingUp } from "lucide-react";
import type { CompetitionSection, IndustrySignalBreakdownItem } from "@/types/opportunity";
...
function statusTheme(status: string, color?: string) {
  ...
  if (c === "green" || c === "high" || c === "favorable" || c === "strong") {
    return {
      ...
      insightBorder: "border-emerald-200",
...
function PricingPowerSection({ ppd }: { ppd: NonNullable<NonNullable<CompetitionSection["text"]>["pricing_power_dynamics"]> }) {
  const items = [
    { label: "Current State", value: ppd.current_state },
    { label: "Shifting Dynamics", value: ppd.shifting_dynamics },
    { label: "Future Trajectory", value: ppd.future_trajectory },
  ].filter((item) => item.value);

  if (items.length === 0 && !ppd.watch_outs) return null;
      ...
      <div className="space-y-2">
        {items.map(({ label, value }) => (
          <div key={label} className="flex items-start gap-2">
            ...
            <div>
              <span className="text-[10px] font-semibold text-[#888888] uppercase tracking-wider mr-1.5">
                {label}
              </span>
              <span style={{ fontSize: 12, color: "#121212", lineHeight: 1.6 }}>{value}</span>
            </div>
          </div>
        ))}

        {ppd.watch_outs && (
          <div className="mt-2 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50/40 px-3 py-2.5">
            <ShieldAlert className="h-3 w-3 text-amber-600 shrink-0 mt-[3px]" />
            <div>
              <span className="text-[10px] font-semibold text-amber-700 uppercase tracking-wider mr-1.5">
                Watch-out
              </span>
              <span style={{ fontSize: 12, color: "#121212", lineHeight: 1.6 }}>{ppd.watch_outs}</span>
            </div>
          </div>
        )}
      </div>
