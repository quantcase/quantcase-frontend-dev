...
import { Users, Info, Globe, Smartphone, Briefcase } from "lucide-react";
import type { CustomerTractionSection, IndustrySignalBreakdownItem } from "@/types/opportunity";
...
function sentimentColors(sentiment: IndustrySignalBreakdownItem["sentiment"]) {
  if (sentiment === "positive")
    return { text: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-200", dot: "bg-emerald-500", fill: "#059669" };
  if (sentiment === "negative")
    return { text: "text-red-600", bg: "bg-red-50", border: "border-red-200", dot: "bg-red-500", fill: "#dc2626" };
  return { text: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", dot: "bg-amber-400", fill: "#D97706" };
}
...
function statusTheme(status: string, color?: string) {
  ...
  if (c === "green" || c === "high" || c === "favorable" || c === "strong") {
    return { border: "#059669", bg: "rgba(5,150,105,0.04)", tagBg: "bg-emerald-600", insightBorder: "border-emerald-200", insightBg: "bg-emerald-50/50" };
  }
  if (c === "red" || c === "low" || c === "unfavorable" || c === "weak") {
    return { border: "#dc2626", bg: "rgba(220,38,38,0.04)", tagBg: "bg-red-600", insightBorder: "border-red-200", insightBg: "bg-red-50/50" };
  }
  return { border: "#D97706", bg: "rgba(217,119,6,0.04)", tagBg: "bg-amber-600", insightBorder: "border-amber-200", insightBg: "bg-amber-50/50" };
}
...
export function CustomerIntelligenceCard({ data }: Props) {
  ...
  const altDataSignals = data.core?.text?.alt_data_signals ?? [];

  const acqItems = data.core?.text?.customer_growth?.acquisition_dynamics ?? [];
  const acqText = acqItems
    .slice(0, 2)
    .map((item) => {
