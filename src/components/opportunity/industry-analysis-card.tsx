        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <p style={{ fontSize: "var(--qc-fz-10)", fontWeight: "var(--qc-w-semi)", fontFamily: "var(--qc-font-sans)", color: borderColor, letterSpacing: "0.05em" }}>
                {s.label.toUpperCase()}
              </p>
              <span style={{ fontSize: "var(--qc-fz-11)", fontWeight: "var(--qc-w-bold)", fontFamily: "var(--qc-font-mono)", color: fill }}>
                {s.score}/{s.max_score}
              </span>
            </div>
            <ul className="space-y-0.5">
              {s.details.map((d, i) => (
                <li key={i} style={{ fontSize: "var(--qc-fz-12)", fontFamily: "var(--qc-font-sans)", color: "var(--qc-ink)", lineHeight: 1.6 }}>• {d}</li>
              ))}
            </ul>
          </div>
        );
      ...
      {entries.map(([key, c]) => {
        ...
        return (
          <div key={key} className="flex items-start gap-2">
            ...
            <p style={{ fontSize: "var(--qc-fz-11)", fontFamily: "var(--qc-font-sans)", color: c.result ? "var(--qc-ink)" : "var(--qc-ink-2)", lineHeight: 1.5 }}>
              {label}
              {c.score > 0 && <span style={{ color: "var(--qc-up)", fontWeight: "var(--qc-w-semi)" }}> +{c.score}</span>}
            </p>
          </div>
        );
      })}
      ...
      <div className="space-y-4">
        {fs?.checks && !Array.isArray(fs.checks) && (
          <div className="rounded-lg p-4 space-y-3" style={{ border: "1px solid var(--qc-hair-2)", background: "var(--qc-card)" }}>
            <p style={{ fontSize: "var(--qc-fz-10)", fontWeight: "var(--qc-w-semi)", fontFamily: "var(--qc-font-sans)", color: "var(--qc-ink-2)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
              Scoring Criteria
            </p>
